...
int main(){
    ...
    printf("Enter nummber for n",n);
    scanf("%d",&n);

    char **board = initialize_board(n);
    
    char symbols[2]={'X','O'}; int current=0,r,c;
    while(1){
        display_board(board,n);
        if(current==0){ printf("Your move (X)\n"); get_human_move(n,board,&r,&c);}
        else { printf("Computer (O)\n"); get_computer_move(n,board,&r,&c); printf("Computer chose %d %d\n",r,c);}
        make_move(board,n,r,c,symbols[current]);
        if(check_win(board,n,symbols[current])){ display_board(board,n); printf("%s wins!\n", current==0?"You":"Computer"); break;}
