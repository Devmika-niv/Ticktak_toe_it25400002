...
int main(){
    int n;
    printf("Type number for N",n);
    scanf("%d",&n);
    char **board = initialize_board(n);
    
    char symbols[3] = {'X', 'O', 'Z'};
    int roles[3] = {0, 0, 0}; // all three players are human
    int r, c, current = 0;

    while (1) {
        display_board(board, n);
        printf("Player %d (%c) %s\n", current + 1, symbols[current],
               roles[current] == 0 ? "Human" : "Computer");
        ...
        if (check_win(board, n, symbols[current])) {
            display_board(board, n);
